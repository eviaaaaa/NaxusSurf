...
from playwright.sync_api import Page as SyncPage, TimeoutError as SyncPlaywrightTimeoutError
...
from langchain_community.tools.playwright.utils import (
    aget_current_page as async_get_current_page,
    get_current_page as sync_get_current_page
)
...
class CaptureElementContextTool(BaseBrowserTool):
    ...
    def _locate_element_sync(self, page: SyncPage, element_description: str) -> Any:
        """同步版本的元素定位逻辑"""
        import re
        ...
        # 策略1：检查标签+ID组合（如 "img#SafeCodeImg"）
        if re.match(r'^[a-z]+[#.][a-zA-Z0-9_-]+$', element_description, re.IGNORECASE):
            try:
                locator = page.locator(element_description)
                if locator.count() > 0:
    ...
    def _extract_surrounding_text_sync(self, page: SyncPage, element: Any) -> str:
        """同步版本的文本提取"""
        try:
    ...
    async def _arun(
        ...
        **kwargs,
        ...
        if self.async_browser is None:
            raise ValueError(f"异步浏览器未提供给 {self.name}")
        
        try:
            # 获取当前页面
            page = await async_get_current_page(self.async_browser)
    ...
    def _run(
        ...
        **kwargs,
        ...
        if self.sync_browser is None:
            raise ValueError(f"同步浏览器未提供给 {self.name}")
        
        try:
            # 获取当前页面
            page = sync_get_current_page(self.sync_browser)
            ...
            # 1. 定位元素
            element = self._locate_element_sync(page, element_description)
            ...
            surrounding_text = ""
            if include_surrounding_text:
                surrounding_text = self._extract_surrounding_text_sync(page, element)
            ...
            # 9. 添加调试覆盖层（如果启用）
            if os.getenv("DEBUG_MODE", "false").lower() == "true":
                self._add_debug_overlay_to_file_sync(
                    screenshot_path, 
                    bounding_box, 
                    capture_area,
                    element_description
    ...
    def _add_debug_overlay_to_file_sync(
        self,
        screenshot_path: Path,
        element_box: Dict[str, float],
        capture_area: Dict[str, int],
